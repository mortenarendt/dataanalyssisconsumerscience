...
Usually, such data is organized in a matrix where each row corresponds to the evaluation of one product by one judge. And the columns are then the attributes. 
...
[NB NB NB nye navne ift det nye Exceldatasæt, så det skal rettes i scripts nedenfor og tjekkes om de rette Excel ark er kopieret med over? Bør vi måske se ud som det vil for de studerende?]
...
[MANGLER: er ovenstående sat op som punkter når man ser det i bogen? Det skal det være]
...
[MORTEN/JULIUS: er det korrekt fortolket af mig? ellers skal der skrives mere til]

[MANGLER: output forklares, julius?]
...
[INDSÆT: link til det rette kapitel ovenfor]
[MANGLER: skal koden ovenfor forklares?]
[MANGLER: output forklares, julius?]

## Cochran's Q test

Cochran's Q test is a statistical test for the comparison of several products, where the response is binary, and there is repeated responses across several judges. We need the package **RVAideeeeMemoire**.The data needs to be structured as the _beercata_ is. 
...
[MANGLER: link til kapitlet om PCA]
