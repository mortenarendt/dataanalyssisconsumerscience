...
To learn more about the pipe operator, see [Edit using Tidyverse]. Here, there is also an explanation of the other functions such as **gather**.
...
pasta %>% 
  gather(Question,
         Answer,
         I_like_taste_of_pasta_with_legumes:Pasta_with_mushrooms_is_visually_appealing) %>%
...
Another nice plot is a bivariate plot, plotting two variables against each other and adding a regression line (using **geom_smooth**, and specifying 'lm' for a linear regression for 'y ~ x'). This has been done below, to see if there is a correlation between the liking and the rating of the visual apprerance for pasta with legumes:
